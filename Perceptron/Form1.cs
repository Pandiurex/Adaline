...
namespace Perceptron
{
    public partial class Form1 : MaterialForm
    {
        ...
        private void Adaline()
        {
            ...
            double errorCuad = 0, errorCuadMax = 1;
            double guess;
            double lr = Convert.ToDouble(learningBox.Text);
            float rangoX = (ciclo > valuatedPanel.Width) ? ciclo / valuatedPanel.Width :
                valuatedPanel.Width / ciclo;
            float rangoY = 1;
            int contador = 0;
            ...
            while (errorCuadMax > Double.Parse(errorBox.Text) && contador < ciclo)
            {
                errorCuadMax = 0;
                for (int i = 0; i < listaP.Count; i++)
                {
                    guess = Sigmoidal((weight[1] * listaP[i].PuntoX) + (weight[2] * listaP[i].PuntoY) - weight[0]);
                    errorCuad = listaP[i].Label - guess;
                    errorCuadMax += Math.Pow(errorCuad, 2) / listaP.Count;

                    weight[0] += lr * errorCuad * (guess * (1 - guess)) * x0;
                    weight[1] += lr * errorCuad * (guess * (1 - guess)) * listaP[i].PuntoX;
                    weight[2] += lr * errorCuad * (guess * (1 - guess)) * listaP[i].PuntoY;
                }

                ListViewItem lista = new ListViewItem(errorCuadMax.ToString());

                errorListView.Items.Add(lista);

                if (contador == 0)
                {
                    rangoY = (valuatedPanel.Height / (float)errorCuadMax);
                }

                evaluated.FillEllipse(brush, (rangoX * contador), valuatedPanel.Height - ((float)(errorCuadMax * rangoY) - 25), 2, 2);
        ...
        private double Sigmoidal(double num)
        {
        ...
        private void colorearPanel()
        {
            ...
            Color plusColor = new Color();
            finalColor = Color.FromArgb(200, 170, 245);
            plusColor = Color.FromArgb(240, 142, 121);
            ...
            for (int x = 0; x < panel1.Width; x += 4)
            {
                for (int y = 0; y < panel1.Height; y += 4)
                {
                    ...
                    brocha = (((weight[1] * x1) + (weight[2] * x2) - weight[0]) < 0) ? new SolidBrush(finalColor) :
                    new SolidBrush(plusColor); ;
        ...
        {
            if (errorBox.Text == "Error Minimo")
            {
                errorBox.Text = "";
            }
        }

        private void adalineBtn_Click(object sender, EventArgs e)
        {
            if (learningBox.Text == "Learning Rate ( η )" || learningBox.Text == "")
            {
                MessageBox.Show("No hay ingresado un valor numerico en el Learning Rate");
            ...
            else if (errorBox.Text == "Error minimo" || errorBox.Text == "")
            {
                MessageBox.Show("No has ingresado un Error minimo");
            }
            else{
